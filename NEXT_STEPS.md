...
- **Factory-spec audit of all 16 CRT profiles against service manuals** — Each `profiles/crts/<id>.json` should hold the actual manufacturer-documented values: dot/grille pitch, native gamma, phosphor chromaticity, scanline frequencies, deflection bandwidth. Today only ~3-4 are verifiably right; the rest are best-effort estimates with NEEDS-MEASUREMENT notes. Sources to chase: official Sony BVM / PVM service manuals (broadcast.sony.com archive), Sony FW900 spec sheet, NEC MultiSync 1/4FG product manuals, Wells-Gardner K7000 / Hantarex installation guides, Sharp Cz-602D/603D/614D Japan service docs, Commodore 1084S Amiga monitor spec, Mac Classic / Lisa B&W panel datasheet. Per profile, the values that need verification: `tube.dot_pitch_mm`, `tube.diagonal_inches`, `tube.aspect_native`, `phosphor.chromaticity_smpte_c`, `phosphor.decay_ms`, `h_freq_khz`, `v_freq_hz`. The current per-profile fine-tuning block in `pipeline.cpp::apply_crt_profile` (PVM, BVM, FW900, etc.) sets *visual* defaults (mask strength, bloom, gamma) by id — those should also be re-derived from the manuals where possible. **In this session**: pvm-8220 pitch corrected 0.50→0.30 mm (Trinitron 8" grille is 0.30 mm per Sony), pvm-20m4 pitch null→0.55 mm (PVM-2030 reference), k7000 pitch null→0.65 mm (arcade shadow mask typical).
