...
struct BackendInitParams {
    ...
    // Phase 4a (D3D12 only): "layered" present mode. Renders to a composition
    // swap chain (no HWND association — CreateSwapChainForHwnd forbids
    // WS_EX_LAYERED windows) but does NOT build a DirectComposition display
    // tree. The overlay instead reads each finished frame back with
    // capture_backbuffer() and paints it onto a WS_EX_LAYERED|TRANSPARENT
    // window via SetLayeredWindowAttributes(LWA_ALPHA) + GDI BitBlt — the
    // only Win32 path that gives cross-process click-through AND shows
    // D3D-rendered content (it mirrors the proven GL recipe; UpdateLayeredWindow
    // would hit-test by per-pixel alpha and swallow clicks). Mutually
    // exclusive with `composition`. GL ignores it.
    bool layered = false;
};
