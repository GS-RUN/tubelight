...
| **4a** | DirectComposition chrome + body separation; drop WS_EX_LAYERED en target mode (por DComp clip) — **PARCIAL** (2026-05-29 `dd95784`): 4a.1 composition swap chain (`CreateSwapChainForComposition` + visual tree) + 4a.2 click-through (`WS_EX_TRANSPARENT`). Verificación por screen-capture (`TUBELIGHT_OVERLAY_CAPTURABLE=1`): `WS_EX_LAYERED` rompía el display (overlay 100% transparente — la superficie DWM layered tapa el visual DComp, justo lo que avisaba este ADR); fix = `WS_EX_TRANSPARENT` solo → display CRT confirmado (glow del bezel visible) + input pass-through. Debug layer 0 errores. **Falta confirmar pass-through de clicks con ratón real** (fallback `WS_EX_NOREDIRECTIONBITMAP` si no cruza procesos) + 4a.3 menú ImGui en DX12. | 1 semana | v0.2.1 |
